...
The only built-time external dependency is libpipeline for handling
...
<cvsroot> is the usual cvs string to identify a cvs repo:
 ...
 For a local repo, just the absolute path.

 For a pserver repo, the string from ~/.cvspass should work.
...
This is made worse by the fact that cvs repos often contain large numbers of tag
create in an attempt to cover up deficiencies in cvs.  [E.g., tag every build,
tag source & target of every merge.]  As a result a large cvs repo may contain
hundreds of millions of file-tags.
...
Like git and cvs, crap-clone is treats text as byte-sequences.  This is
transparent to all character sets, but has the down-side of leaving you guessing
as to what character set is in use.  If you can do better than that, patches
welcome.
...
Not at present.  I use bare git archives purely for the cvs import, and then
git-fetch/git-pull into a separate archive for anything else.  Patches welcome.
...
where 'cvs tag -b' is used to add files to an existing branch; we have to guess
which files are branched late and when they were branched.]
...
doesn't give enough information and does not cope with the case where a detected
merge requires reordering of commits.
